 ...
 */
//var pagebz= null;
function initGrid(form,events,pagebz){  
	...
	var ipage  = "input[name='page']";
	var ipages = "input[name='pages']"
	var irows  = "select[name='rows']";
	var inrows = "input[name='rows']";
	var apage  = "a[name=page]";
	
	if(pagebz!=null && pagebz!='undefined'){
		 ipage  = "input[name='"+pagebz+"_"+"pageNum']";
		 ipages = "input[name='"+pagebz+"_"+"pages']"
		 irows  = "select[name='"+pagebz+"_"+"pageSize']";
		 inrows = "input[name='"+pagebz+"_"+"pageSize']";
		 apage  = "a[name="+pagebz+"_"+"pageNum]";
	}
	
	
	
	var page  = form.find(ipage);
	var pages = form.find(ipages);
	var rows  = form.find(irows);
	if (rows.length == 0) rows = form.find(inrows);
	...
	form.find(apage).click(function() {
		page.val($(this).text());
		switchButton();
		form.submit();
	});
